...
- Hook: `plugin.generate({ irs: Array<ImplodeIRRefs<FileIR>> })`
...
- Hook: `plugin.derive({ irr: IRRef<FileIR, SourceFile> })`
- Paired with the specific FileIR being processed
...
**⚠️ Status:** Hook interfaces are evolving. Currently `codegen`, `generate`, and `derive` hooks are available.
