...
```typescript
// Default slot filtering
lightChildren.filter((node) => {
  if (node.nodeType === Node.ELEMENT_NODE) {
    return !node.hasAttribute("slot");  // Exclude named slot elements
  }
  return true;  // Include text and comments
});

// Named slot filtering
lightChildren.filter((node) => {
  if (node.nodeType === Node.ELEMENT_NODE) {
    return node.getAttribute("slot") === slotName;
  }
  return false;  // No text nodes for named slots
});
```
