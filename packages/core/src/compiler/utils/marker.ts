...
export function tryFindPencelMarker(
  ...
  try {
    const marker = JSON.parse(comment);
...
export function isPencelSourceUpToDate(tsFile: ts.SourceFile): boolean {
  const marker = tryFindPencelMarker(tsFile);

  if (!marker) {
    return false;
  }

  const nowIntegrity = sourceFileSha256(tsFile);
